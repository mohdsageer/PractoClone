...
const SearchBar = () => {
  ...
  const [showDoctorListData, setShowDoctorListData] = useState(false);

  const dummyLocation = ["Bangalore", "Pune", "Mumbai", "Delhi", "Hydrebad"];

  const DoctorList = ["Dentist", "Gynecologist", "Obstetrician"];
  let loadallDataHandler = () => {
    setLocation(dummyLocation);
    setDoctorArray(DoctorList);
  };
  useEffect(() => {
    loadallDataHandler();
  }, []);
  ...
  const locationHandler = (e) => {
    if (e.target.value) {
      ...
      setSelectedLocation(e.target.innerHTML);
    }
  };
  ...
  const doctorHandler = (e) => {
    if (e.target.value) {
      ...
      setDoctors(e.target.innerHTML);
    }
  };

  const BringLocationListHandler = (e) => {
    setShowLocationData(true);
    setShowDoctorListData(false);
  };

  let onDoctorFocusHandler = (e) => {
    setShowLocationData(false);
    setShowDoctorListData(true);
  };

  return (
    <div className="container">
      <div id="searchbar_wrapper" className={"searchBarWrapper"}>
        <div className={"searchBarContainer"}>
          <div className={"searchBarLocality"}>
            <div className={"searchbox_wrapper"}>
              <span className={"searchbox_icon"}>
                <svg
                  width="13"
                  height="17"
                  viewBox="0 0 13 17"
              ...
              <input
                ...
                className={"searchbox"}
                type="text"
                id={"searchbox_locality"}
                placeholder={"Search Location"}
                onChange={locationHandler}
                onFocus={BringLocationListHandler}
                onMouseEnter={BringLocationListHandler}
              />
              {showLocationData && location && (
                <div className="locationListContainer">
                  <ul onMouseLeave={() => setShowLocationData(false)}>
          ...
          <div className={"searchBarKeyword"}>
            <div className={"searchbox_wrapper"}>
              <span className={"searchbox_icon"}>
                <svg
                  width="19"
              ...
              <input
                ...
                onFocus={onDoctorFocusHandler}
                onMouseEnter={onDoctorFocusHandler}
              />
              {showDoctorListData && doctorArray && (
                <div className="locationListContainer">
                  <ul onMouseLeave={() => setShowDoctorListData(false)}>
                    {doctorArray.map((name, index) => (
                      <li
                        className="locationList"
                        key={index}
                        onClick={(e) => doctorHandler(e)}
                      >
                        {name}
                      </li>
                    ))}
